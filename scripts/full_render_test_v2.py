...
async def execute_full_test():
    try:
        ...
        print(f"🔍 Fetching scripts to find: {campaign_id}")
        
        all_scripts = []
        async for d in mongo.db.scripts.find({}):
            all_scripts.append(d.get("campaign_id"))
        print(f"📋 Available scripts in DB: {all_scripts}")

        script_doc = await mongo.db.scripts.find_one({"campaign_id": campaign_id})
        if not script_doc:
            # Try fuzzy match or case-insensitive if not found
            script_doc = await mongo.db.scripts.find_one({"campaign_id": {"$regex": f"^{campaign_id}$", "$options": "i"}})
            if script_doc:
                print(f"⚠️ Found script via fuzzy match: {script_doc.get('campaign_id')}")

        if not script_doc:
            print(f"❌ No script document found for {campaign_id} in {all_scripts}")
            return
        ...
        state = {
            ...
            "campaign_id": script_doc.get("campaign_id"),
        ...
        print(f"🚀 Render Starting for campaign: {state['campaign_id']}...")
        ...
        print(f"❌ Exception in full test: {e}")
        traceback.print_exc()
    finally:
