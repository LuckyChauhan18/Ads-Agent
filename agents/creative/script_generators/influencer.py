...
class InfluencerScriptGenerator(BaseScriptGenerator):
    """Script Generator for Influencer-style ads."""
    ...
    def generate_script_llm(self, language="Hindi", platform="Instagram Reels", ad_length=30):
        """Custom story type script generator for Influencer-style."""
        ...
        rules_str = "\n".join([f"- {rule}" for rule in common_rules]) if common_rules else "- Focus on being energetic, authentic and relatable."
...
1. The Influencer/Avatar MUST speak to camera or narrate with high energy.
2. Follow the template scene count ({len(template_scenes) if template_scenes else 7} scenes).
3. Visual Continuity: Ensure the Avatar or action is recognizable across scenes.
...
{template_structure_desc if template_structure_desc else 'Standard Influencer structural flow'}
...
1. **Avoid Generic Phrases**: Do NOT use terms like 'best performance', 'ultimate experience', 'high quality'. Use specific, visual, in-action scenario language (e.g. 'Hooks attention in 2 seconds').
...
4. **Product/Visual Consistency**: Visual elements must remain identical across all scenes (same products, branding, ambient atmosphere). No redesign variation allowed.
5. **Realism Guideline**: Avoid perfect CGI look. Add slight camera shake, natural micro-imperfections, and realistic physics interactions.
...
[
  {{
    ...
    "visual": "Influencer talking to camera, relatable setup.",
